    ...
    ** VARCHAR2 주의사항 **
    -오라클은 기본적으로 UTF-8 문자 인코딩을 사용하는데 
     ...
     EX) VARCHAR2(3)  : 가변길이 문자형 크기 3BYTE
     		 NVARCHAR2(3) : 가변 길이 문자형 문자 3개까지 저장 가능
     		 만약 한글 2000자 -> 6000BYTE > 4000BYTE 로 오류 발생
...
INSERT INTO "MEMBER"
VALUES('MEM01','123ABC', '홍길동', '990101-1234567',CURRENT_DATE);
-- 아이디, 비밀번호, 이름 주민등록번호, 가입일

COMMIT; --> 트랜잭션 저장 내용 DB반영
...
--    USER_ID VARCHAR2(20) UNIQUE, -- 컬럼 레벨 제약 조건(제약 조건명 x) 
    ...
    USER_PWD VARCHAR2(30) , -- 테이블 레벨로 제약조건 설정하기
    USER_NAME VARCHAR2(30),
    GENDER VARCHAR2(10),
    PHONE VARCHAR2(30),
    EMAIL VARCHAR2(50),
...
INSERT INTO USER_USED_UK
VALUES(1, NULL, 'pass01', '나길동', '남', '010-1234-5678', 'hong123@kh.or.kr');
--> NAME의 무결성 제약조건 (KH_CWC.USER_NAME_U)에 위배됩니다( 중복 )
...
-- ORA-00001: 무결성 제약 조건(KH_CWC.USER_ID_NAME_U)에 위배됩니다
...
CREATE TABLE USER_USED_PK(
    USER_NO NUMBER CONSTRAINT USER_NUMBER_PK PRIMARY KEY, -- 컬럼레벨 설정
    ...
    -- 테이블 레벨 설정
    -- , CONSTRAINT USER_NUMBER_PK PRIMARY KET (USER_NO)
);
...
--> 기본키 중복으로 오류
--> ORA-00001: 무결성 제약 조건(KH_CWC.USER_NUMBER_PK)에 위배됩니다

INSERT INTO USER_USED_PK
VALUES(NULL, 'user03', 'pass03', '유관순', '여', '010-9999-3131', 'yoo123@kh.or.kr');
--> 기본키가 NULL 이므로 오류
--> ORA-01400: NULL을 ("KH_CWC"."USER_USED_PK"."USER_NO") 안에 삽입할 수 없습니다


---------------------------------------

-- PRIMARY KEY 복합키 (테이블 레벨만 가능) , 모두 중복이여야 오류 , NULL 삽입 시 오류
...
--> ORA-00001: 무결성 제약 조건(KH_CWC.PK_USERNO_USERID)에 위배됩니다
...
--> ORA-01400: NULL을 ("KH_CWC"."USER_USED_PK2"."USER_NO") 안에 삽입할 수 없습니다

----------------------------------------------------------------------------------------------------------------

/* 관계형 데이터베이스 핵심 */
-- 4. FOREIGN KEY(외부키 / 외래키) 제약조건 
 -- 자식 테이블 컬럼에 저장될 수 있는 값은 부모 테이블 컬렘에 작성된 값만 저장 가능
...
CREATE TABLE USER_GRADE(
  GRADE_CODE NUMBER PRIMARY KEY, -- 식별자 역할 
...
CREATE TABLE USER_USED_FK(
  ...
  GRADE_CODE NUMBER -- 해당 컬럼에 작성될 수 있는 값은
  									-- USER_GRADE 테이블의 
  									--PK 컬럼값(GRADE_CODE)만 작성하게 제한하고 싶다!
  									-- FK 제약조건 설정
...
-- 오류 : ORA-02291: 무결성 제약조건(KH_CWC.GRADE_CODE_FK1)이 위배되었습니다- 
 			-- 부모 키가 없습니다
...
FROM USER_GRADE
WHERE GRADE_CODE = 10;
-- ORA-02292: 무결성 제약조건(KH_CWC.GRADE_CODE_FK1)이 위배되었습니다
-- 자식 레코드가 발견되었습니다
...
SELECT * FROM USER_GRADE2; -- 10 값 사라짐
SELECT * FROM USER_USED_FK2; -- 자식 테이블 의 10 값이 NULL 로 변함
...
SELECT * FROM USER_GRADE3; -- 10 제거 됨
SELECT * FROM USER_USED_FK3; -- 10 값을 가진 행 자체 제거
...
CREATE TABLE USER_USED_CHECK(
  ...
  GENDER VARCHAR2(10) 
  	...
  	CHECK( GENDER IN ('남','여') ),
  	--> GENDER컬럼에 저장되는 값이 '남' 또는 '여' 중 하나인지 검사
...
-- ORA-02290: 체크 제약조건(KH_CWC.GENDER_CHECK)이 위배되었습니다
...
-- 2) JOIN 후 원하는 컬럼만 테이블로 복사 ( AS 후 괄호 시 오류 )
