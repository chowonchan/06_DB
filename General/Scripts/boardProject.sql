...
SELECT 
	...
	(SELECT COUNT(*)
	 FROM BOARD_LIKE L
	 WHERE L.BOARD_NO = 2017) AS LIKE_COUNT,
	 
	 (SELECT IMG_PATH || IMG_RENAME
	  FROM BOARD_IMG I
	  WHERE I.BOARD_NO = 2017
...
-- AND 와 OR 해석 순서
--AND BOARD_TITLE LIKE '%'||'11'||'%' -- AND 가 먼저 해석되고
--OR BOARD_CONTENT LIKE '%'||'11'||'%'; 
-- 다른 테이블 인거 처럼 BOARD_CODE가 인식 되지 않는 점이 발생
...
COMMENT ON COLUMN "NOTIFICATION"."SEND_MEMBER_NO" IS '알림 보낸회원 번호(SEQ_MEMBER_NO)';
COMMENT ON COLUMN "NOTIFICATION"."RECEIVE_MEMBER_NO" IS '알림 받는 회원 번호(SEQ_MEMBER_NO)';
...
-- SUBSTR(컬럼명, 시작 인덱스) : 인뎃그 부터 끝까지 잘라서 반환
-- INSTR(컬럼명, '찾을문자열', 검색 순서) : 일치하는 문자열 인덱스 반환
SELECT 
		SUBSTR(PROFILE_IMG, 
...
COMMENT ON COLUMN "CHATTING_ROOM"."CREAT_DATE" IS '채팅방 생성일';
...
SELECT CHATTING_ROOM_NO
			...
			,(SELECT COUNT(*) FROM MESSAGE M WHERE M.CHATTING_ROOM_NO = R.CHATTING_ROOM_NO AND READ_FL = 'N' AND SENDER_NO != 12) NOT_READ_COUNT
