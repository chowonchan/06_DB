...
WHERE BONUS IS NOT NULL 	 -- 2. 어떤 행만 조회 할지 지정
GROUP BY DEPT_CODE				 -- 3. WHERE 결과에서 그룹 구분
ORDER BY DEPT_CODE ASC;		 -- 5. 조회된 결과 정렬
...
--> EMPLOYEE 테이블에서 DEPT_CODE 기준으로 만들진 그룹 중
...
GROUP BY DEPT_CODE
ORDER BY DEPT_CODE ASC;
--> 개인별 급여가 350만 이상인 사람들의 부서별 평균, 부서 평균 x(잘못된 SQL)
...
FROM EMPLOYEE
GROUP BY JOB_CODE
ORDER BY JOB_CODE ASC; -- 7행 조회(보너스 0명인 조회결과 포함)
...
INTERSECT -- 공통된 결과를 가진 대북혼, 심봉선만 조회됨

-- 급여가 400만 초과 하는 사원의 이름 ,부서코드, 급여 조회
SELECT EMP_NAME, DEPT_CODE, SALARY
FROM EMPLOYEE
...
MINUS -- 공통된 결과를 가진 대북혼, 심봉선을 뺀 나머지 결과 조회
