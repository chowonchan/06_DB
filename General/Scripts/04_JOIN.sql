...
-- ORA-00918: 열의 정의가 애매합니다 
	--> JOB_CODE 컬럼명이 같아서 어떤 테이블의 컬럼인지 구불할 수 없기 때문

--> 해결방법 : JOB_CODE 앞에 테이블명 또는 별칭 작성
...
SELECT 
		EMP_ID,
		EMP_NAME,
		E.JOB_CODE, -- 어떤 테이블의 컬럼을 조회할지 별칭을 작성
		JOB_NAME
...
JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
JOIN LOCATION ON (LOCATION_ID = LOCAL_CODE)
WHERE JOB_NAME = '대리' AND LOCAL_NAME LIKE 'ASIA%'; -- 차태연 , 전지연 
