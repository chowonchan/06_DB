...
-- ALTER(바꾸다, 변조하다)
-- 수정 가능한 것 : 컬럼(추가/수정/삭제), 제약조건(추가/삭제) 수정 불가
--                  이름변경(테이블, 컬럼, 제약조건)
...
SELECT * FROM DEPT_COPY;
--> 새로 추가된 LNAME 컬럼에 기본값 '한국'이 자동으로 세팅
...
-- (수정 에러 상황 : 저장된 값보다 작은 크기 로 변환 불가)
...
ALTER TABLE DEPT_COPY
DROP COLUMN DEPT_ID; -- 전부 사라지면 테이블의 정의가 다 사려져서
...
---------------------------------------
/* DML + DDL 혼용 사용하기 */
INSERT INTO DEPT_COPY -- DML
VALUES('D0'); -- 1행 삽입

SELECT * FROM DEPT_COPY; -- 'D0' 추가 됨

-- DEFT_COPY 테이블에 컬럼 추가(DDL 수행)
ALTER TABLE DEPT_COPY
...
-- 테이블 삭제 시 연결되어있는 FK제약 조건을 같이 삭제 시킴
