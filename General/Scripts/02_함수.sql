...
-- (indexOf 와 비슷)
-- INSTR(문자열 | 컬럼명, '찾을 문자열' [, 찾을 시작 위치 [, 순번]]) 대괄호 생략 가능
...
-- 문자열에서 5번 부터 검색 시작해서 두 'B'의 위치 조회
SELECT 'AABAACAABBAA', 
...
FROM "NATIONAL"; -- "테이블 명" == 이름이 "테이블명"과 같은 테이블
...
-- CEIL(천장)(숫자 | 컬럼명)  : 올림 -> 정수 형태로 반환
-- FLOOR(층)(숫자 | 컬럼명)	: 내림 -> 정수 형태로 반환
...
SELECT 
		123.456,
		ROUND(123.456), 		-- 123 / 지정된 소수점 위치 없으면 첫째 자리
		ROUND(123.456, 0),  -- 123  (0이 ROUND 기본 값 (.) 을 기준)
		ROUND(123.456, 1),  -- 123.5 소수점 1의 자리 까지 표현 , 소수점 2째자리를 반올림
...
-- <날짜 관련 함수>
-- SYSDATE : 현재 시간 (시스템, DB가 설치된 컴퓨터)
-- SYSTIMESTAMP : 현재 시간 (ms 포함, 표준시간대)

-- CURRENT_DATE (접속한 계정,접속 위치 시간)
-- CURRENT_TIMESTAMP
SELECT 
...
SELECT 
		MONTHS_BETWEEN( TO_DATE('2024-08-16', 'YYYY-MM-DD') , CURRENT_DATE)
FROM DUAL;
...
-- EX) "123" + 123 -> 246
SELECT 
		MONTHS_BETWEEN('2024-12-06', '2024-6-24'),
		(TO_DATE('2024 -12 -06') - TO_DATE('2024-6-24')) /30
FROM DUAL; -- 5.42 == 5개월 13일

/* MONTH_BETWEEN이 지정되 두 날짜 사이의 차를 계산할 때 
 * 훨씬 좋다!!(더 정확함)
 * -> 달 마다 길이(28, 29, 30, 31)가 다 다르기 때문에
 * 		직접 게산하면 오차가 있을 수 있는데
 * 		MONTHS_BETWEEN을 이용하면 이런 부분까지 모두
 * 		자동으로 적용되어 계산된다!!
 * 
 * */
 ...
 * 3) L : 현재 시스템이나 DB에 설정된 나라의 화폐 기호 (원화)
 * 4) , : 숫자의 자릿수 구분
 * */
...
FROM DUAL; -- #### 지정된 숫자 칸수 보다 변환 칸수 적음
...
SELECT TO_CHAR(CURRENT_DATE, 'YY"년" MM"월" DD"일" DAY AM HH"시" MI"분" SS"초"')
FROM DUAL;
...
-- 50이상 확인
SELECT
		TO_DATE('51-12-25','YY-MM-DD'), -- 2051
...
-- DECODE(컬럼명 |(OR) 계산식, 조건1, 결과1, 조건2, 결과2, ... [,아무것도 만족 X])
...
SELECT
		...
		CASE
			...
			WHEN SALARY >=4000000 THEN '중급' -- 이미 600만 이상이 고급으로 지정되어 600미만 작성 불필요
			ELSE '초급'
		END 구분
...
SELECT COUNT(DISTINCT DEPT_CODE) -- 중복 제거
FROM EMPLOYEE;
