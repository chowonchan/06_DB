...
-- 기본적으로 "서브 쿼리"를 먼저 해석
	--> 서브쿼리 결과를 이요해서"메인 쿼리"해석

-- 단, 상호 연관 서브쿼리는 순서가 반대(메인-> 서브)
...
/* GROUP BY 에 작성된 컬럼명만 SELECT절에 작성할 수 있다!!! */
...
JOIN JOB USING (JOB_CODE)
WHERE (LOCATION_ID, JOB_CODE) = (
			SELECT LOCATION_ID, JOB_CODE
			FROM EMPLOYEE
			WHERE EMP_NAME = '노옹철'
			)
AND EMP_NAME != '노옹철';
...
FROM EMPLOYEE E
JOIN JOB ON (JOB.JOB_CODE = E.JOB_CODE)
...
-- JOB_CODE 의 각각의 이름이 부여되면 JOB 의 JOB_CODE의 위치가 애매해지기때문에
-- 비교를 위해 JOB에 있는 JOB_CODE 에도 별칭 부여하여 다른 JOB_CODE 와 비교하기위해
-- ON 을 사용 하여야 한다
